...
def get_dashboard_stats(db: Session = Depends(get_db)):
    # 1. Total produits calculé en SQL
    ...
    # 3. Points de vente
    total_pos = db.query(func.count(PointOfSale.id)).scalar() or 0
    active_pos = db.query(func.count(PointOfSale.id)).filter(PointOfSale.is_active == True).scalar() or 0
    
    # 4. Alerte stock bas (uniquement les produits concernés avec leur catégorie en JOIN)
    low_stock_products = db.query(Product).options(joinedload(Product.category)).filter(Product.quantity <= Product.min_stock_alert).all()
    low_stock_count = len(low_stock_products)
    
    # 5. Derniers mouvements avec jointures préchargées (zéro requête N+1)
