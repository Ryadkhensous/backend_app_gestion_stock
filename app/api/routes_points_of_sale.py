...
def get_nearby_points_of_sale(
    ...
    all_pos = db.query(PointOfSale).filter(PointOfSale.is_active == True).all()
    nearby = []
    for pos in all_pos:
...
def get_point_of_sale(pos_id: int, db: Session = Depends(get_db)):
...
def create_point_of_sale(payload: PointOfSaleCreate, db: Session = Depends(get_db)):
    pos = PointOfSale(**payload.model_dump())
    db.add(pos)
    db.commit()
    db.refresh(pos)
    return pos
...
def update_point_of_sale(pos_id: int, payload: PointOfSaleUpdate, db: Session = Depends(get_db)):
    pos = db.query(PointOfSale).filter(PointOfSale.id == pos_id).first()
    if not pos:
        raise HTTPException(status_code=404, detail="Point de vente introuvable.")
...
def delete_point_of_sale(pos_id: int, db: Session = Depends(get_db)):
    pos = db.query(PointOfSale).filter(PointOfSale.id == pos_id).first()
    if not pos:
        raise HTTPException(status_code=404, detail="Point de vente introuvable.")
    db.delete(pos)
